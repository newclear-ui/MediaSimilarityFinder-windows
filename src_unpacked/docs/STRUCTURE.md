...
> 실제 소스는 `src_unpacked/`에 있다. 저장소 루트 `C:\project`에는 매 빌드 산출물
> (소스 zip, 포터블 zip, 바이너리 zip)이 쌓인다. 모든 작업은 `src_unpacked/` 기준으로 수행한다.
...
- `gui/` — Qt 위젯 앱. `main.cpp`(kVersion 포함), `mainwindow.h/.cpp`(한글 포함, 반드시 Edit 도구로 수정),
  `video_decoder.h/.cpp`.
...
| `video_sampling.*` | 동영상 샘플 플랜(득점 간격, `sampling_interval`) |
| `video_decoder.h` | FFmpeg(MSF_HAS_FFMPEG) 디코더 인터페이스 |
| `video_fingerprint.*` | 동영상 지문: timestamps+hashes+mirror 캐시(versioned, SQLite), 저분산 프레임 필터 + 장면전환, `video_similarity`(DTW, sceneBonus) |
| `similarity.*` | 해시 유사도 / 이미지 매칭 |
| `candidate_index.*` | 유사 후보 색인(빅 O 개선) |
| `scan_pipeline.*` | 중복 검색 엔진: duration 게이트 + temporal 동영상 검증(스캔 경로) |
| `media_search_engine.*` | 스캔/증분/라이브 매칭 오케스트레이션, GPU 카운터 |
| `database.*` | SQLite 파일 상태/지문 저장 |
...
- `mainwindow.cpp` — 모든 위젯/트리스트/상태바, 한글 문자열은 `trStr()` 테이블(파일에 한국어 리터럴 존재).
- 미리보기: `fileThumb()` — IThumbnailCache(WTS_INCACHEONLY) → video는 VideoDecoder, image는 QImageReader → WIC → 파일 아이콘.
- 상태바: 진행률(스캔 중) + GPU 라벨(`gpuLbl_`, 스캔 중 'GPU 처리 N', 정지 시 on/off).

## 빌드/테스트

- `scripts/build_windows_cuda.ps1 -VcpkgRoot C:\src\vcpkg` — CUDA 빌드.
- `ctest --test-dir build -C Release` — 전체 테스트(공식 기준선 이후 항상 green 유지).
- `MediaSimilarityFinder.exe --smoke` (offscreen), `--version` — GUI 스모크/버전 확인.
- 버전 상향 파일(검색용): `CMakeLists.txt`, `vcpkg.json`, `gui/main.cpp`, `scripts/package_portable.ps1`, `src/index_manager.cpp`, `gui/mainwindow.cpp`(trStr about + buildUi + applyStaticTexts).

## 문서 갱신 규칙(AGENTS.md)

- 빌드마다 `docs/build-history/<버전>.ko.md` + `.en.md` 쌍, `docs/build-history/README.ko/.en.md` 버전표.
