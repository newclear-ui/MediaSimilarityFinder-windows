...
QString trStr(UiLang lang, const char* key) {
  ...
  if (!std::strcmp(key,"about")) return S("Media Similarity Finder 0.9.4.8\n미디어 중복/유사 검색 (CPU/GPU)\n언어: 설정에서 한국어/English 전환","Media Similarity Finder 0.9.4.8\nMedia duplicate/similarity search (CPU/GPU)\nLanguage: switch 한국어/English in Settings");
...
void MainWindow::buildUi() {
  setWindowTitle(trStr(lang(), "app") + QStringLiteral(" 0.9.4.8"));
  resize(1500, 880);
...
void MainWindow::applyStaticTexts() {
  const UiLang l = lang();
  setWindowTitle(trStr(l, "app") + QStringLiteral(" 0.9.4.8"));
