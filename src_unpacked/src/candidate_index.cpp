...
unsigned CandidateIndex::partitionBits(unsigned){ return 16; }
unsigned CandidateIndex::partitionShift(unsigned part){ return part*16; }
CandidateIndex::BucketKey CandidateIndex::key(std::uint64_t h,unsigned part){
  return BucketKey{static_cast<std::uint8_t>(part),static_cast<std::uint16_t>((h>>(part*16))&0xFFFFu)};
}
template <typename F> static void forRadius2(std::uint16_t v,F&& fn){
  fn(v);
  for(unsigned i=0;i<16;++i) fn(static_cast<std::uint16_t>(v^(1u<<i)));
  for(unsigned i=0;i<16;++i) for(unsigned j=i+1;j<16;++j) fn(static_cast<std::uint16_t>(v^(1u<<i)^(1u<<j)));
}
...
std::vector<Candidate> CandidateIndex::query(std::uint64_t h,unsigned maxDistance) const{
  ...
  if(maxDistance>8){queryBKFallback(h,maxDistance,r);} else {
    ...
    if(maxDistance==0){
      ...
      candidates.reserve(entries_.size()/16+64);
      for(unsigned p=0;p<kPartitions;++p){
        const std::uint16_t pv=static_cast<std::uint16_t>((h>>(p*16))&0xFFFFu);
        forRadius2(pv,[&](std::uint16_t nb){
          const auto it=buckets_.find(BucketKey{static_cast<std::uint8_t>(p),nb});
          if(it!=buckets_.end())candidates.insert(candidates.end(),it->second.begin(),it->second.end());
        });
      }
    }
    ...
    for(std::size_t pos=0;pos<entries_.size();++pos){
      ...
      for(unsigned part=0;part<partitionCount;++part){
        if(maxDistance==0){
          ...
          const std::uint16_t pv=static_cast<std::uint16_t>((e.hash>>(part*16))&0xFFFFu);
          forRadius2(pv,[&](std::uint16_t nb){
            const auto it=buckets_.find(BucketKey{static_cast<std::uint8_t>(part),nb});
